...
function ACBDashboard() {
  ...
  const fetchCardData = async (selectedYear: string) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/API/Count/Dashboard/Details?report_id=2&year=${selectedYear}`
      );
      if (res.status === 200) {
  ...
  const fetchOverdueData = async (selectedYear: string) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/API/List/Dashboard/Overdue/Report?page=0&size=1000&report_id=2&year=${selectedYear}`
      );
      if (res.status === 200) {
